  ...
  return (
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-2 md:p-4 font-sans z-10">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-start pointer-events-auto gap-2 w-full max-w-full">
        ...
        <div className="bg-gray-900/90 text-white p-2 md:p-3 rounded-xl border border-gray-700 shadow-2xl backdrop-blur-md flex gap-3 md:gap-6 items-center justify-between md:justify-start w-full md:w-auto">
          ...
          <div className="flex flex-col items-end">
             <span className="text-[8px] md:text-[10px] text-gray-400 uppercase font-bold tracking-widest">Tasks</span>
             <span className="text-base md:text-lg font-bold text-white font-mono">{stats.missionCount}</span>
          </div>
        </div>

        {/* Hyperlocal Search */}
        <div className="bg-gray-900/90 text-white p-2 rounded-xl border border-gray-700 shadow-2xl backdrop-blur-md flex gap-2 items-center pointer-events-auto">
          <div className="flex flex-col">
            <span className="text-[8px] text-gray-400 uppercase font-bold tracking-widest">Hyperlocal SG</span>
            <input 
              type="text" 
              placeholder="Postal Code" 
              value={postalCode}
              onChange={(e) => setPostalCode(e.target.value.replace(/\D/g, '').slice(0, 6))}
              className="bg-black/50 border border-gray-700 rounded px-2 py-1 text-xs font-mono focus:border-green-500 outline-none w-24"
            />
          </div>
          <button 
            onClick={() => onSearchPostalCode(postalCode)}
            disabled={postalCode.length !== 6}
            className="bg-green-600 hover:bg-green-500 disabled:opacity-50 disabled:hover:bg-green-600 p-2 rounded-lg transition-all"
            title="Search Postal Code"
          >
            <Radar className="w-4 h-4" />
          </button>
          <button 
            onClick={() => onSearchPostalCode((Math.floor(Math.random() * 800000) + 100000).toString())}
            className="bg-blue-600 hover:bg-blue-500 p-2 rounded-lg transition-all"
            title="Jump to Random Area"
          >
        ...
        <div className="pointer-events-auto flex gap-2">
            <button 
                ...
                className="bg-[#00b14f] text-white p-2 md:p-3 rounded-xl border border-green-400 shadow-lg backdrop-blur-md flex gap-2 items-center hover:bg-[#009241] transition-all relative"
            >
                <Package className="w-5 h-5 md:w-6 md:h-6" />
                <div className="flex flex-col items-start leading-tight">
                    <span className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest">Grab Missions</span>
                    <span className="text-xs md:text-sm font-black">{availableMissions.length} Available</span>
                </div>
                {availableMissions.length > 0 && (
                    <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center text-[10px] animate-bounce">
      ...
      <div className="absolute bottom-1 right-2 md:right-4 text-[8px] md:text-[9px] text-white/30 font-mono text-right pointer-events-auto hover:text-white/60 transition-colors">
        <a href="https://grab.com" target="_blank" rel="noreferrer" className="text-green-400/50 hover:text-green-400">Powered by GRAB MAPS</a>
      </div>
        ...
        {showMissions && (
            ...
            >
                <motion.div 
                    ...
                    animate={{ scale: 1, y: 0 }}
                    exit={{ scale: 0.9, y: 20 }}
                    className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[80vh]"
                    onClick={e => e.stopPropagation()}
                >
                    <div className="bg-[#00b14f] p-6 text-white flex justify-between items-center">
                        <div>
                            <h2 className="text-2xl font-black italic tracking-tighter flex items-center gap-2 uppercase">
                                <Radar className="w-6 h-6" />
                                Grab Mission Center
                            </h2>
                            <p className="text-xs font-bold opacity-80 uppercase tracking-widest">Fulfill Drone deliveries in SkyMetropolis</p>
                        </div>
                        <button onClick={() => setShowMissions(false)} className="hover:bg-black/20 p-2 rounded-full">
                            <X className="w-6 h-6" />
                        </button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        {availableMissions.length === 0 ? (
                            <div className="text-center py-12 text-slate-400 flex flex-col items-center">
                                <Radar className="w-12 h-12 mb-2 opacity-20 animate-pulse" />
                                <p className="font-bold">Scanning for new tasks...</p>
                                <p className="text-[10px] uppercase opacity-50">Local businesses are busy!</p>
                            </div>
                        ) : (
                            availableMissions.map(m => (
                                <div key={m.id} className="bg-slate-800 rounded-2xl p-4 border border-slate-700 hover:border-green-500/50 transition-all group">
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="flex items-center gap-2">
                                            <div className="p-2 bg-green-500/20 rounded-lg text-green-400">
                                                {m.type === 'delivery' ? <Package className="w-5 h-5" /> : <Radar className="w-5 h-5" />}
                                            </div>
                                            <h3 className="font-bold text-white text-lg">{m.title}</h3>
                                        </div>
                                        <div className="text-yellow-400 font-mono font-black border border-yellow-400/30 px-2 py-0.5 rounded text-sm bg-yellow-400/5">
                                            +${m.reward}
                                        </div>
                                    </div>
                                    ...
                                    <button 
                                        ...
                                        className="w-full bg-white text-black font-black py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-green-500 hover:text-white transition-all uppercase text-sm tracking-widest group-hover:scale-[1.02]"
                                    >
                                        Dispatch Drone
                    ...
                    <div className="p-4 border-t border-slate-800 bg-slate-800/50 flex items-center justify-between">
                        <div className="flex items-center gap-2 text-[10px] text-slate-400 uppercase font-black">
                            ...
                            Active Drones: {stats.missionCount} total
                        </div>
                        <div className="flex items-center gap-2 text-[10px] text-green-400 uppercase font-black">
                            <CheckCircle2 className="w-3 h-3" />
                            Powered by Grab Maps
                        </div>
                    </div>
                </motion.div>
            </motion.div>
